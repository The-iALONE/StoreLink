...
class AdminOrderService {
	 ...
	 */
	public function list_page( int $page = 1, string $status = '' ): array {
		$page = max( 1, $page );
		$args = array(
			'limit'    => self::PER_PAGE,
			'page'     => $page,
	...
	public function get( int $order_id ): ?array {
		...
		$summary['edit_url'] = $order->get_edit_order_url();
		$summary['has_download'] = $order->has_downloadable_item();
	...
	public function update_status( int $order_id, string $status ): \WP_Error|\WC_Order {
		...
		$allowed = array( 'pending', 'on-hold', 'processing', 'completed', 'cancelled', 'refunded', 'failed' );
	...
	private function summary( \WC_Order $order ): array {
		return array(
			...
			'has_download'  => $order->has_downloadable_item(),
		);
	}

	public static function normalize_status( string $status ): string {
		$status = sanitize_key( $status );
		$ok     = array( 'pending', 'processing', 'on-hold', 'completed', 'cancelled', 'refunded', 'failed' );
		return in_array( $status, $ok, true ) ? $status : '';
	}
}
